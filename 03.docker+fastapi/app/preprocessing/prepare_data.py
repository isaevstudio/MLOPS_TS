...
def df_date_convert(folder_path, saving_path=None):
...
def from_csv_summarize(df, symbol, folder_path=None, saving_path=None):
...
def from_csv_sentiment(df, model, tokenizer, device, folder_path=None, saving_path=None):
...
def fill_missing_dates_with_exponential_decay(df, date_column, sentiment_column, decay_rate=0.05):
    ...
    full_df[sentiment_column].fillna(0, inplace=True)
    return full_df
...
def integrate_data(stock_price_df, news_df):
    ...
    if df_cleaned.loc[df_cleaned['sentiment'] == 0].shape[0] != 0:
...
#prepare data
"""
def create_sequences(data, seq_length, predict_horizon):
    xs, ys = [], []
    if predict_horizon == 1:
        predict_horizon = 0
        for i in range(len(data) - seq_length - predict_horizon):
            x = data[i:i+seq_length]
            y = data[i+seq_length:i+seq_length+predict_horizon, 0]  # Close price as target
            xs.append(x)
            ys.append(y)
